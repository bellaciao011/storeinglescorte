...
import { Lock, RefreshCw, CheckCircle2, Clock, Package, Truck, Home, Mail, LogOut } from "lucide-react";
...
export default function Admin() {
  ...
  const totalRevenue = orders
    ...
    .reduce((s, o) => s + Number(o.amount_eur), 0);
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-[#6b0f1a] text-white px-4 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => fetchOrders(savedPw)}
            className="bg-white/10 hover:bg-white/20 p-2 rounded-lg"
          >
            <RefreshCw className={`w-4 h-4 ${loading ? "animate-spin" : ""}`} />
          </button>
          <button
            onClick={() => { setAuthed(false); setSavedPw(""); setOrders([]); }}
            className="bg-white/10 hover:bg-white/20 p-2 rounded-lg"
          >
                  ...
                  <div className="flex flex-wrap items-start gap-4">
                    ...
                    {order.payment_status === "paid" && (
                      <div className="flex items-center gap-2 flex-shrink-0">
                        ...
                        {saving !== order.id && order.order_status === "delivered" && (
